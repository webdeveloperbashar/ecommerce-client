...
const Index = () => {
  ...
  const catePlainText = category.split('-').join(" ")
  ...
  }, [dispatch, searchValues, category]);
  useEffect(() => {
  ...
  return (
    <>
      <Helmet>
        <title>{`${catePlainText}`} - GreenValleyGrocery Shop</title>
      </Helmet>
      {device.isDesktop && <Nav isShow />}
      <div className="shop__wrapper">
        <div className="shop__section__image searchByCategory__image">
          <h2 className="text-center text-white">{catePlainText}</h2>
        </div>
              ...
              <div className="shop__sidebar mt-5">
                ...
                <div className="products__tag box__shadow p-4 bg-light mt-3">
                  ...
                  <div className="tags__wrapper">
                    {["fruits", "kashmiri", "apple", "virgin"].map((item) => (
                      <NavLink to={`/searchByTag/${item}`}>{item}</NavLink>
                    ))}
                  </div>
                </div>
              </div>
